...
fn status(f: &mut Frame<CrosstermBackend<io::Stdout>>, rect: Rect, active: bool) {
    let block = Block::default().title("").borders(Borders::NONE);
    let mut text = Spans::from(Span::styled(
        "PROCESANDO",
        Style::default().fg(Color::Green),
    ));
    if !active {
        text = Spans::from(Span::styled("TERMINADO", Style::default().fg(Color::Gray)));
    }
    f.render_widget(
        Paragraph::new(text)
            .block(block)
...
fn time(f: &mut Frame<CrosstermBackend<io::Stdout>>, rect: Rect, msg: &str, time: u32) {
    ...
    let text = Spans::from(vec![
        Span::from(msg),
        Span::from(types::seconds_to_str(time)),
    ]);
    f.render_widget(
        Paragraph::new(text)
            .block(block)
